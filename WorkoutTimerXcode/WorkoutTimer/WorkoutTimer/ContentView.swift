...
class AudioEngine {
    ...
    func beep(freq: Double = 880, duration: Double = 0.15, volume: Float = 0.4) {
        DispatchQueue.global(qos: .userInteractive).async {
            ...
            self.play(samples: data, sampleRate: sampleRate)
        }
    }
    ...
    func playDone() {
        ...
        for (i, f) in freqs.enumerated() {
            DispatchQueue.main.asyncAfter(deadline: .now() + Double(i) * 0.18) {
                self.beep(freq: f, duration: 0.2, volume: 0.5)
            }
        }
    }
    ...
    func playTick(isLast: Bool) {
        beep(freq: isLast ? 1100 : 660, duration: 0.12, volume: 0.35)
    }
...
class WorkoutViewModel: ObservableObject {
    ...
    func startSession() {
        ...
        sessionTimer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { [weak self] _ in
            ...
            if self.sessionLeft <= 1 {
                self.sessionLeft = 0
                self.sessionFired = true
    ...
    func roundDone() {
        ...
        rounds += 1
        if enableRest {
            let restSecs = restMin * 60
            restCountdown = restSecs
            phase = .rest
            restTimer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { [weak self] _ in
                ...
                if self.restCountdown <= 1 {
                    ...
                    if self.restCountdown <= 5 {
                        AudioEngine.shared.playTick(isLast: self.restCountdown == 1)
                    }
                }
            }
        } else {
...
extension Color {
    static let accent = Color(red: 1, green: 0.42, blue: 0.21)       // #FF6B35
    static let green  = Color(red: 0.19, green: 0.82, blue: 0.35)    // #30D158
    static let bg     = Color(red: 0.067, green: 0.067, blue: 0.067) // #111
    static let bg2    = Color(red: 0.09, green: 0.09, blue: 0.09)    // #171717
    static let bg3    = Color(red: 0.1, green: 0.1, blue: 0.1)       // #1a1a1a
    static let ring   = Color(red: 0.118, green: 0.118, blue: 0.118) // #1e1e1e
...
struct RingButton: View {
    ...
    var accentColor: Color { phase == .countdown ? .accent : .green }

    var strokeOffset: CGFloat {
        switch phase {
        case .countdown: return 0
        case .rest: return circumference * CGFloat(1 - restPct)
    ...
    var body: some View {
        Button(action: onTap) {
            ZStack {
                ...
                // Progress ring
                Circle()
                    .trim(from: 0, to: phase == .countdown ? 1 : CGFloat(phase == .rest ? restPct : 0))
                    .stroke(accentColor, style: StrokeStyle(lineWidth: 14, lineCap: .round))
                    .frame(width: size, height: size)
                    .rotationEffect(.degrees(-90))
                    .animation(.linear(duration: 1), value: restPct)
                ...
                GeometryReader { geo in
                    ZStack(alignment: .leading) {
                        ...
                        RoundedRectangle(cornerRadius: 2)
                            ...
                            .animation(.linear(duration: 1), value: vm.sessionPct)
                    }
                }.frame(height: 4)
            }
            .padding(.horizontal, 20)
            ...
            HStack(spacing: 6) {
                ForEach(0..<max(vm.rounds + 1, 1), id: \.self) { i in
                    Circle()
                        ...
                        .overlay(
                            Circle().stroke(i == vm.rounds && vm.phase == .countdown ? Color.accent : Color.clear, lineWidth: 1)
                        )
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Text("Workout Complete")
                .font(.system(size: 26, weight: .bold)).foregroundColor(.white).padding(.bottom, 4)
            Text("Great effort!")
                .font(.system(size: 13)).foregroundColor(.dim).padding(.bottom, 32)
